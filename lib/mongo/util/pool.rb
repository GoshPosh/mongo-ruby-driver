        ...
        @connection_mutex.synchronize do
          if socket_for_thread = thread_local[:sockets][self.object_id]
            if !@checked_out.include?(socket_for_thread)
              ...
              puts "#{Time.now.strftime('%Y-%m-%d %H:%M:%S')} Mongo: Socket already checked out: Thread id: #{Thread.current.object_id} client: #{@client.inspect}, pool: #{self.inspect}, socket: #{socket_for_thread} "
            end
          else
